...
<details>
  ...
  <img src="image/screenshot.png" alt="项目截图" style="max-width:600px">
</details>
...
- [Vercel 一键部署](https://vercel.com/new/clone?repository-url=https%3A%2F%2Fgithub.com%2FKUAILESHANGWEI%2FLibreTV)
- [Netlify 一键部署](https://app.netlify.com/start/deploy?repository=https://github.com/KUAILESHANGWEI/LibreTV)
- [Render 一键部署](https://render.com/deploy?repo=https://github.com/KUAILESHANGWEI/LibreTV)
...
本仓库是通过非 Fork 方式建立的独立镜像，安装、更新、运行和部署入口统一指向 [KUAILESHANGWEI/LibreTV](https://github.com/KUAILESHANGWEI/LibreTV)。

仓库内的 GitHub Actions 会自动同步上游当前最新 Release 的全部 Assets 到本仓库 Release（见 `.github/workflows/sync.yml`）。如需同步上游代码，请先在独立分支验证再合并，避免覆盖本仓库的本地化改造。
...
```bash
curl -fsSL https://raw.githubusercontent.com/KUAILESHANGWEI/LibreTV/main/install.sh | PASSWORD=your_password bash
```
...
docker run -d \
  ...
  ghcr.io/kuaileshangwei/libretv:latest
```
...
services:
  libretv:
    image: ghcr.io/kuaileshangwei/libretv:latest
